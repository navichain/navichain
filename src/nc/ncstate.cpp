...
#include <nc/ncstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

NcState::NcState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = NcState::openDB(_path + "/ncDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

NcState::NcState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute NcState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, NcTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createNcAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            nc::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> NcState::vins() const // temp
{
...
void NcState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* NcState::vin(dev::Address const& _a) const
{
    return const_cast<NcState*>(this)->vin(_a);
}

Vin* NcState::vin(dev::Address const& _addr)
{
...
// void NcState::commit(CommitBehaviour _commitBehaviour)
// {
...
void NcState::kill(dev::Address _addr)
{
...
void NcState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address NcState::createNcAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void NcState::deleteAccounts(std::set<dev::Address>& addrs){
...
void NcState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void NcState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
