...
bool NcDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule NcDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t NcDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t NcDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t NcDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t NcDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool NcDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void NcDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void NcDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void NcDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void NcDGP::createParamsInstance(){
...
dev::Address NcDGP::getAddressForBlock(unsigned int blockHeight){
...
void NcDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void NcDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void NcDGP::parseStorageOneUint64(uint64_t& value){
...
void NcDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule NcDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
