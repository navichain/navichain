...
const char * const BITCOIN_CONF_FILENAME = "nc.conf";
const char * const BITCOIN_PID_FILENAME = "ncd.pid";
...
bool OpenDebugLog()
{
    ...
    assert(fileoutVM == nullptr); // nc
    assert(vMsgsBeforeOpenLog);
    fs::path pathDebug = GetDebugLogPath();
    fs::path pathDebugVM = GetDebugVMLogPath(); // nc
    fileout = fsbridge::fopen(pathDebug, "a");
    fileoutVM = fsbridge::fopen(pathDebugVM, "a"); // nc
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Nc
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Nc
    // Mac: ~/Library/Application Support/Nc
    // Unix: ~/.nc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nc";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Nc";
#else
    // Unix
    return pathRet / ".nc";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("NaviChain") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The NaviChain developers";
    }
