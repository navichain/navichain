#include <qt/nrctoken.h>
#include <qt/forms/ui_nrctoken.h>
...
NRCToken::NRCToken(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::NRCToken),
    m_model(0),
    m_clientModel(0),
...
void NRCToken::setModel(WalletModel *_model)
{
    m_model = _model;
...
void NRCToken::setClientModel(ClientModel *_clientModel)
{
...
void NRCToken::on_goToSendTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(0);
}

void NRCToken::on_goToReceiveTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(1);
}

void NRCToken::on_goToAddTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(2);
}

void NRCToken::on_currentTokenChanged(QModelIndex index)
{
    if(m_tokenModel)
    {
        if(index.isValid())
...
void NRCToken::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void NRCToken::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void NRCToken::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void NRCToken::contextualMenu(const QPoint &point)
{
