...
#define paternVersion "nc-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

NcVersionChecker::NcVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

NcVersionChecker::~NcVersionChecker()
{

}

bool NcVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> NcVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(NC_RELEASES)));
    QEventLoop event;
...
Version NcVersionChecker::getMaxReleaseVersion()
{
