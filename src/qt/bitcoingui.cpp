    ...
    platformStyle(_platformStyle)
{
    ...
    ncVersionChecker = new NcVersionChecker(this);

    if(fCheckForUpdates && ncVersionChecker->newVersionAvailable())
    {
        QString link = QString("<a href=%1>%2</a>").arg(NC_RELEASES, NC_RELEASES);
        QString message(tr("New version of Nc wallet is available on the Nc source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Nc address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and nc: URIs)"));
    ...
    NRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/nrctoken"), tr("&NRC Tokens"), this);
    NRCTokenAction->setStatusTip(tr("NRC Tokens (send, receive or add Tokens in list)"));
    NRCTokenAction->setToolTip(NRCTokenAction->statusTip());
    NRCTokenAction->setCheckable(true);
    NRCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    tabGroup->addAction(NRCTokenAction);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Nc addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Nc addresses"));
    ...
    openAction->setStatusTip(tr("Open a nc: URI or payment request"));//NC_LINE

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Nc command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->mapGroup(NRCTokenAction, tokenActions);
...
void BitcoinGUI::gotoSendTokenPage()
{
    ...
    NRCTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Nc network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Nc"); // default title
