...
bool ToHash160(const std::string& strNcAddress, std::string& strHash160)
{
    CTxDestination ncAddress = DecodeDestination(strNcAddress);
    if(!IsValidDestination(ncAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&ncAddress);
    if(keyid){
...
bool ToNcAddress(const std::string& strHash160, std::string& strNcAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination ncAddress = keyid;
    if(IsValidDestination(ncAddress)){
        strNcAddress = EncodeDestination(ncAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToNcAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToNcAddress(tokenEvent.receiver, tokenEvent.receiver);
