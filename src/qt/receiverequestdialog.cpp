...
#include <config/bitcoin-config.h> /* for USE_NRCODE */
#endif
    ...
    model(0)
{
    ...
    ui->lblNRCode->setVisible(false);
#endif

    connect(ui->btnSaveAs, SIGNAL(clicked()), ui->lblNRCode, SLOT(saveImage()));
}
...
bool ReceiveRequestDialog::createNRCode(QLabel *label, SendCoinsRecipient _info, bool showAddress)
{
#ifdef USE_NRCODE
...
#ifdef USE_NRCODE
    if(createNRCode(ui->lblNRCode, info))
    {
        ui->lblNRCode->setScaledContents(true);
