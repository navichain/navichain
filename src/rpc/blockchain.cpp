...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // nc
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // nc
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // nc
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // nc
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CTxDestination ncSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(ncSenderAddress)) {
            const CKeyID *keyid = boost::get<CKeyID>(&ncSenderAddress);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of nc addresses\n"
