...
#include <nc/storageresults.h>


extern std::unique_ptr<NcState> globalState;
...
using ExtractNcTX = std::pair<std::vector<NcTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // nc
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashNcRoot,
    const dev::h256& oldHashStateRoot, const std::vector<NcTransaction>& transactions);
...
public:
    ...
    NcTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionNcTransactions(ExtractNcTX& ncTx);
...
private:
    ...
    NcTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<NcTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
