...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setNcSchedule(ncDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = ncDGP.getBlockSize(nHeight);
    minGasPrice = ncDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = ncDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    NcTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractNcTX resultConverter;
    if(!convert.extractionNcTransactions(resultConverter)){
    ...
    for(NcTransaction ncTransaction : ncTransactions){
        txGas += ncTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + ncTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(ncTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, ncTransactions, hardBlockGasLimit);
...
void StakeNcs(bool fStake, CWallet *pwallet)
{
