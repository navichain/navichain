...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ncproject/nc>";
    const std::string URL_WEBSITE = "<https://vuacoin.com>";
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                dev::eth::Ethash::init();
                fs::path ncStateDir = GetDataDir() / "stateNc";
                bool fStatus = fs::exists(ncStateDir);
                const std::string dirNc(ncStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsNcstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<NcState>(new NcState(dev::u256(0), NcState::openDB(dirNc, hashDB, dev::WithExisting::Trust), dirNc, existsNcstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::ncMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(ncStateDir.string()));
                if (fReset) {
