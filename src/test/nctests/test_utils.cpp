...
void initState(){
    ...
    const std::string dirNc = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<NcState>(new NcState(dev::u256(0), NcState::openDB(dirNc, hashDB, dev::WithExisting::Trust), dirNc + "/ncDB", dev::eth::BaseState::Empty));
...
NcTransaction createNcTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    NcTransaction txEth;
    if(recipient == dev::Address()){
        txEth = NcTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = NcTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<NcTransaction> txs){
