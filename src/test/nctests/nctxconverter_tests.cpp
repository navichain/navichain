...
void checkResult(bool isCreation, std::vector<NcTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    NcTxConverter converter(transaction, NULL);
    ExtractNcTX ncTx;
    BOOST_CHECK(converter.extractionNcTransactions(ncTx));
    std::vector<NcTransaction> result = ncTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(nctxconverter_tests, TestingSetup)
