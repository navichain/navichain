...
#if defined(NDEBUG)
# error "Nc cannot be compiled without assertions."
#endif
...
std::unique_ptr<NcState> globalState;
...
const std::string strMessageMagic = "Nc Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = ncDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = ncDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            NcTxConverter converter(tx, NULL);
            ExtractNcTX resultConverter;
            if(!converter.extractionNcTransactions(resultConverter)){
            ...
            std::vector<NcTransaction> ncTransactions = resultConverter.first;
            std::vector<EthTransactionParams> ncETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(NcTransaction ncTransaction : ncTransactions){
                sumGas += ncTransaction.gas() * ncTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, ncTransaction.gasPrice());
                } else {
                    txMinGasPrice = ncTransaction.gasPrice();
                }
                VersionVM v = ncTransaction.getVersion();
                if(v.format!=0)
                ...
                if(ncTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(ncTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(ncTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)ncTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(ncETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > ncTransactions.size())
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s NC additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // nc
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // nc
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    NcTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<NcTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path ncDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(ncDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(ncDir.string(), std::ios::in | std::ios::out);
...
bool NcTxConverter::extractionNcTransactions(ExtractNcTX& nctx){
    std::vector<NcTransaction> resultTX;
    ...
    nctx = std::make_pair(resultTX, resultETP);
    return true;
}

bool NcTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool NcTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
NcTransaction NcTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    NcTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = NcTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = NcTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // nc
    NcDGP ncDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setNcSchedule(ncDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = ncDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = ncDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = ncDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // nc
...
///////////////////////////////////////////////////////////////////////////////////////// nc
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            NcTxConverter convert(tx, &view, &block.vtx);

            ExtractNcTX resultConvertNcTX;
            if(!convert.extractionNcTransactions(resultConvertNcTX)){
            ...
            if(!CheckMinGasPrice(resultConvertNcTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertNcTX.first, blockGasLimit);
            ...
            for(NcTransaction& qtx : resultConvertNcTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertNcTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertNcTX.first[k].from(), resultConvertNcTX.first[k].to(),
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // nc
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // nc
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // nc
            globalState->setRootUTXO(oldHashUTXORoot); // nc
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // nc
        uint32_t sizeBlockDGP = ncDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Nc PoS we need to sync this after every block to ensure txdb is populated for
