...
*Setup instructions for a Gitian build of NaviChain using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Nc
...
- [Building NaviChain](#building-nc-core)
...
Clone the git repositories for nc and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ncproject/nc --recursive
...
```bash
sudo cp /home/debian/nc/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the nc repository under 'Fetch and create inputs' to install sources which require
...
To build NaviChain (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the nc repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/nc/.git/
...
For example:
```bash
URL=https://github.com/laanwj/nc.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit nc=${COMMIT} --url nc=${URL} ../nc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit nc=${COMMIT} --url nc=${URL} ../nc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit nc=${COMMIT} --url nc=${URL} ../nc/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-nc:
./bin/gbuild --commit nc=${COMMIT},cpp-eth-nc=${ETHCOMMIT} --url nc=${URL},cpp-eth-nc=${ETHURL} ../nc/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the nc git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../nc/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/nc
SIGPATH=/some/root/path/nc-detached-sigs

./bin/gbuild --url nc=${BTCPATH},signature=${SIGPATH} ../nc/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nc-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nc-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/nc-osx-build.assert
```
...
[ncproject/gitian.sigs](https://github.com/ncproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@vuacoin.com) and he will commit them.